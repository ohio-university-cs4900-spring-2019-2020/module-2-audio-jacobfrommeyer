...
void GLViewNewModule::onCreate()
{
   ...
   //At this point, all the managers are initialized. That is, the engine is fully initialized.
    ...
    std::string audio_file_1 = "../media/Subwoofer_Lullaby.ogg";
   ...
   this->audio = audio_mgr::init();
   this->audio->play_2D_audio("../media/Subwoofer_Lullaby.ogg", true, false, true);
   std::cout << "setting 2d audio volume" << std::endl;
   this->audio->get_2D_audio().at(0)->setVolume(1.0f);
   std::cout << "starting 3d audio" << std::endl;
   this->audio->play_3D_audio("../media/oof.ogg", Vector(50, 0, 3), true, false, true);
   this->audio->get_3D_audio().back()->setVolume(0.9f);
   this->audio->get_3D_audio().back()->setMinDistance(5.0f);
}
...
void GLViewNewModule::updateWorld()
{
   ...
   //this->audio->setListenerPosition(irrklang::vec3d(loc.x, loc.y, loc.z), irrklang::vec3d(view.x, view.y, view.z));

   Aftr::Vector view = cam->getLookDirection();
   Aftr::Vector location = cam->getPosition();
   this->audio->get_audio_engine()->setListenerPosition(irrklang::vec3d(location.x, location.y, location.z), irrklang::vec3d(view.x, view.y, view.z));
}
